...
exports.submit = function (client, text) {
  ...
  if (my.game.winner.indexOf(client.id) === -1 &&
    ...
    if (my.game.primary === 0) {
      if (my.game.roundTime - t > 10000) { // 가장 먼저 맞힌 시점에서 10초 이내에 맞히면 점수 약간 획득
        clearTimeout(my.game.qTimer)
        ...
        for (let i in my.game.robots) {
          if (my.game.roundTime > my.game.robots[i]._delay) {
            ...
            if (client !== my.game.robots[i]) {
              if (Math.random() < ROBOT_CATCH_RATE[my.game.robots[i].level]) {
  ...
  if (play) {
    if (my.game.primary + my.game.giveup.length >= my.game.seq.length) {
      clearTimeout(my.game.hintTimer)
      clearTimeout(my.game.hintTimer2)
      clearTimeout(my.game.qTimer)
      my.turnEnd()
