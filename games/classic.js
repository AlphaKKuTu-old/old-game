    ...
    */
    if (title == null) {
      ...
      Lizard.all(list).then(function (res) {
        for (let i in res) if (!res[i]) return R.go(EXAMPLE)
...
exports.submit = function (client, text) {
  ...
  function onDB ($doc) {
    ...
    if ($doc) {
      ...
      else if (!my.opts.character && ($doc.flag & Const.KOR_FLAG.CHARACTER)) denied(408)
      else preApproved()
    } else {
      denied()
    }
  ...
  function isChainable () {
    ...
    let char = my.game.char
    let subChar = my.game.subChar
    let l = char.length
